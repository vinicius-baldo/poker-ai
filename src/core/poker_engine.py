...
class GameState:
    ...
    def add_player(self, name: str, stack: int, position: int):
    ...
    def record_action(self, player: str, action: Action, amount: int = 0):
...
class HandEvaluator:
    ...
    @classmethod
    def evaluate_hand(cls, hole_cards: List[Card], community_cards: List[Card]) -> Tuple[int, str]:
        """
        ...
        class_string = cls.evaluator.class_to_string(cls.evaluator.get_rank_class(score))
        return (score, class_string)

    @classmethod
    def calculate_equity(cls, hole_cards: List[Card], community_cards: List[Card], opponent_range: List[str]) -> float:
        """
