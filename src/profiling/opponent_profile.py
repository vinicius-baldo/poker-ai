...
class OpponentProfile:
    ...
    def __init__(self, name: str):
        ...
        self.hands_at_showdown: List[List[str]] = []  # List of hands shown at showdown (e.g., ['Ah', 'Kd'])
        self.action_history: List[Dict] = []  # List of actions per hand

    def record_hand(self, actions: List[Dict], went_to_showdown: bool = False, showdown_hand: Optional[List[str]] = None):
    ...
    def get_stats(self) -> Dict:
        ...
        stats = {
            ...
            'vpip_pct': (self.vpip / self.hands_played * 100) if self.hands_played else 0,
            'pfr_pct': (self.pfr / self.hands_played * 100) if self.hands_played else 0,
            'three_bet_pct': (self.three_bet / self.hands_played * 100) if self.hands_played else 0,
            'aggression_factor': (self.aggressive_actions / self.calls) if self.calls else 0,
            'showdown_pct': (self.showdowns / self.hands_played * 100) if self.hands_played else 0,
            'wtsd_pct': (self.went_to_showdown / self.hands_played * 100) if self.hands_played else 0,
    ...
    def from_json(data: str) -> 'OpponentProfile':
        ...
        profile.three_bet = int(stats.get('three_bet_pct', 0) * profile.hands_played / 100)
        profile.aggressive_actions = int(stats.get('aggression_factor', 0) * profile.calls)
        profile.calls = profile.calls or 1  # Avoid division by zero
        profile.showdowns = int(stats.get('showdown_pct', 0) * profile.hands_played / 100)
        profile.went_to_showdown = int(stats.get('wtsd_pct', 0) * profile.hands_played / 100)
