...
class CardTemplate:
    ...
    def __init__(
        self, rank: Rank, suit: Suit, template: np.ndarray, confidence: float = 0.8
    ) -> None:
        self.rank = rank
        self.suit = suit
...
class CardRecognizer:
    ...
    def __init__(self, template_dir: str = "data/card_templates") -> None:
    ...
    def load_templates(self) -> None:
        ...
        if not os.path.exists(self.template_dir):
            logger.warning(
                f"Template directory {self.template_dir} not found. Creating it."
            )
    ...
    def create_template(
        self, card: Card, image: np.ndarray, region: Tuple[int, int, int, int]
    ) -> None:
    ...
    def recognize_card(
        ...
        if best_match:
            logger.debug(
                f"Recognized card {best_match} with confidence {best_score:.3f}"
            )
            return best_match
    ...
    def calibrate_card_regions(
        self, image: np.ndarray, known_cards: List[Card]
...
class TemplateManager:
    ...
    def create_all_templates(self, reference_image: str) -> None:
        """
